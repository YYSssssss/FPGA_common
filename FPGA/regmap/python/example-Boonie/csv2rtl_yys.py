...
print(csv_scr_dir)
#csv_scr_dir = 'C:/Users/YYS/Desktop/gen_reg/'
xls_file=csv_scr_dir+'/sensemi_prototype_verification_v_0_0_1.xlsx'
...
CheaderBaseFile='sensemi_logan_base_address.h'
addr_bits='11'

#BasecsvFile = 'LoganBaseAddressTable.csv'

hdr='"sensemi_'+project_name+'_params.svh"'
...
hdrFile = "sensemi_header.svh"
hdrData = open(hdrFile, 'w',encoding="utf-8",errors='ignore')
...
def gen_sig_def_block(csvFile, intfName, incl_stat):
    ...
    #print(csvFile)
    CsvData = csv.reader(open(csvdir_path+'/'+csvFile,encoding="utf-8",errors='ignore'))
    next(CsvData)
    sig_def_block=''
    file_name=intfName.replace('IFP','sensemi')
    intf_sig_def_block=("// -------------------------------------------------------------------------- // \n"+  
                "// Copyright Jabil Inc.                                                       // \n" +
                "// -------------------------------------------------------------------------- // \n" +
...
def gen_rtl_logic(csvFile):
    ...
    rtlFile = "sensemi_"+((csvFile.lower()).replace(".csv","_regs.sv"))
    rtlFile = (rtlFile.replace("top_ctrl_",""))
    intfFile = "sensemi_"+((csvFile.lower()).replace(".csv","_ctrl_if.sv"))
    ...
    #print(rtlFile)
    rtlData = open(gen_rtl_path+'/'+rtlFile, 'w',encoding="utf-8",errors='ignore')
    intfData = open(gen_if_path+'/'+intfFile, 'w',encoding="utf-8",errors='ignore')
    intfStatData = open(gen_if_path+'/'+intfFile.replace('ctrl','stat'), 'w',encoding="utf-8",errors='ignore')
...
def gen_c_base_hdr_file(BasecsvFile):
    ...
    ChdrBaseData = open(gen_c_hdr_path+'/'+CheaderBaseFile, 'w',encoding="utf-8",errors='ignore')
    
    ChdrBaseData.write("\n\n//////////////////  BASE ADDRESS defines /////////////////////////////////////////////////////////////\n\n")
    
    CsvBase = csv.reader(open(csvdir_path+'/'+BasecsvFile,encoding="utf-8",errors='ignore'))
    next(CsvBase)
...
def gen_c_hdr_file(csvFile):
    ...
    CsvData = csv.reader(open(csvdir_path+'/'+csvFile,encoding="utf-8",errors='ignore'))
    next(CsvData)
    CHdrFile = "sensemi_"+((csvFile.lower()).replace(".csv",".h"))
    ChdrData = open(gen_c_hdr_path+'/'+CHdrFile, 'w',encoding="utf-8",errors='ignore')
    c_reg_addr=''
    prev_reg=''
    for row in CsvData:
        if(row[reg_name_idx]!=prev_reg):
...
def main():
    ...
    print("Converting sensemi_prototype_verification_v_0_0_1.xlsx to csv")
    ...
    if (os.path.isfile(xls_file)):
       ...
       if (platform.system()=='Windows'):
           os.system("python xls2csv_v2.py sensemi_prototype_verification_v_0_0_1.xlsx")
