...
user= getpass.getuser()                                     #获取当前主机用户名

csv_scr_dir = os.getcwd()                                   #获取当前文件夹路径

xls_file=csv_scr_dir+'/Mintaka_regmap_bringup_210512.xlsx'  #输入要处理的Excel文件路径

csvdir_path=csv_scr_dir+'/csv'                              #定义生成CSV文件的路径
gen_rtl_path=csv_scr_dir+'/gen_rtl'                         #定义gen_rtl文件的路径
gen_if_path=csv_scr_dir+'/gen_if'                           #定义gen_if文件的路径
gen_c_hdr_path=csv_scr_dir+'/gen_c_hdr'                     #定义gen_c_hdr_path文件的路径
...
BasecsvFile = 'Mintaka_regmap_bringup_210512.csv'
# BasecsvFile = 'LoganBaseAddressTable.csv' #LoganSub6Registers.xlsx

hdr='"jb_'+project_name+'_params.svh"'
ignore_warning_stat_RW=['dl_ant_postcfr_gain_fraction', 
...
reg_type_idx=4                                              #
fld_name_idx=5                                              #
bit_width_idx=6                                             #Excel的bit_width要在第七列(必须)
rst_value_idx=7                                             #
desc_idx=8                                                  #
...
def gen_sig_def_block(csvFile, intfName, incl_stat):
    global header_param                                     #函数内部需要调用外部变量,需要用global声明
    check_errors(csvFile)                                   #调用check_errors函数
    CsvData = csv.reader(open(csvdir_path+'/'+csvFile))     #打开csvFile文件并变为CsvData对象
    next(CsvData)                                           #返回CsvData对象顶部
    sig_def_block=''                                        #定义一个sig_def_block文本对象
    file_name=intfName.replace('IFP','jb')                  #更改文件名,将IFP变为jb
...
def main():
    ...
    print("Converting Mintaka_regmap_bringup_210512.xlsx to csv")
    if not os.path.exists(csvdir_path):
       os.makedirs(csvdir_path)
    if (os.path.isfile(xls_file)):
       print("Found the xls File  "+xls_file+"\n")
       if (platform.system()=='Windows'):
           a=os.system("python xls2csv_v2.py Mintaka_regmap_bringup_210512.xlsx")
