...
def find_filelist_fpga(filelist_fpga_path):
    ...
    matching_files = []
    for f in files:
        if f.lower().endswith('filelist_fpga') or f.lower().endswith('x_includeformat_filelist'):
...
def gen_synplify_project(vobs_path,filelist_path,file_syn_tcl_path,synplify_project_name,
    ...
    need_delate_word3 = "+define+INCLUDE_DATA_FORMAT+FPGA+FOURLANES"
    need_delate_word4 = "+define+INCLUDE_DATA_FORMAT+FPGA"
    need_delate_word5 = "+define+NOT_INCLUDE_DATA_FORMAT+FPGA+FOURLANES"
    need_delate_word6 = "+define+NOT_INCLUDE_DATA_FORMAT+FPGA"
    header0 = "header files are defined in"
    need_operate_words = [need_delate_word0,need_delate_word1,need_delate_word2,
                          need_delate_word3,need_delate_word4,need_delate_word5,
                          need_delate_word6
                         ]
...
#--  Project file /home/users/yuyushan/work/LGV1R3C01_B002/synplify/""" + synplify_project_name + '.prj \n')
        ...
        f2.write('set_option -include_path' + vobs_path + '/cores/mipi_radar/src/ \n')
        f2.write('set_option -hdl_define -set FPGA=1 \n\n\n')
        f2.write('set_option -hdl_define -set FPGA_ILA=1 \n\n\n')
        # f2.write('set_option -hdl_define -set INCLUDE_DATA_FORMAT=1 \n\n\n')
        f2.write('set_option -hdl_define -set NOT_INCLUDE_DATA_FORMAT=1 \n\n\n')
        ...
        for root,dirs,files in os.walk(adder_stub_v_path):
            for file in files:
                name,extension = os.path.splitext(file)
                if "stub" in name and extension == ".v":
                    file_path = os.path.join(root,file)
        ...
        for root,dirs,files in os.walk(mipi_v_path):
            for file in files:
                name,extension = os.path.splitext(file)
                if "stub" in name and extension == ".v":
                    file_path = os.path.join(root,file)
...
if __name__ == "__main__":
    ...
    adder_stub_v_path = input_synplify_folder_path + "../synplify_xilinx_rtl/ip/adder"
    mipi_v_path = input_synplify_folder_path + "../synplify_xilinx_rtl/ip/mipi"
