    ...
    for i in range(len(result_complex)):
        ...
        if isOut32b==0:
            ...
            if if_rfft:
                ...
                if real_value>2**15:
                    real_value=-2**16+real_value;
                real_bytes = struct.pack("<l", real_value)
                real_bytes = int.from_bytes(real_bytes, byteorder='little', signed=True)
                real_dec = Fxp(real_bytes, signed=True, n_word=16, n_frac=0, overflow='saturate',rounding='floor')
                real_bytes = struct.pack("<h", int(real_dec))

                if imaginary_value>2**15:
                    imaginary_value=-2**16+imaginary_value;
        ...
        # 将实部和虚部转换为32位有符号数
            if real_value > 2 ** 31:
                real_value = -2 ** 32 + real_value;

            if imaginary_value > 2 ** 31:
                imaginary_value = -2 ** 32 + imaginary_value;
...
#把对比结果用16进制存在txt里面
def generate_comp_data(result_complex,output_file,if_rfft,is_32):
    ...
    with open(output_file, 'w') as file:
        if if_rfft==1:
            for i in range(0, len(arr_list),cmp_line * 2):
                # 获取四个元素并颠倒顺序
                temp_str = ''.join(reversed(arr_list[i:i + cmp_line * 2]))
                # 转换为十六进制并添加0
                hex_str = hex(int(temp_str, 2))[2:]
                file.write(hex_str + '\n')

        else:
            for i in range(0, len(arr_list), cmp_line):
                # 获取四个元素并颠倒顺序
                temp_str = ''.join(reversed(arr_list[i:i + cmp_line]))
                # 转换为十六进制并添加0
...
def read_complex_data(input_file,if_rfft):
    ...
    for i, line in enumerate(lines):
        ...
        if len(hex_numbers[0]) < 32:
            zero_bytes = b'0' * (32 - len(hex_numbers[0]))
            hex_numbers = [zero_bytes + num for num in hex_numbers]

        # Now, you can join the bytes together
        hex_numbers_str = b''.join(hex_numbers).decode('utf-8')
